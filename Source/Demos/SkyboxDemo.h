...
public:
    void Start(Graphics& graphics) override
    {
        ...
        cubeMesh = new Mesh(cubeVertices, std::move(cubeIndices), graphics);
        planeMesh = new Mesh(std::move(planeVertices), std::move(planeIndices), graphics);
        skyboxMesh = new Mesh(cubeVertices, std::move(skyboxIndices), graphics);
        ...
        skyboxShader = new Shader(L"Skybox.vs.cso", L"Skybox.ps.cso", graphics);
        ...
        skyboxTexture = new Texture(L"..\\..\\Data\\skybox.dds", graphics);
        ...
        lp.m_Lights[0].m_Direction = XMFLOAT4(0.5f, -1, -0.75f, 0);
